...
class PrometheusMetricsMixin(ScenarioMetrics):
    ...
    def scrape_duration(self, scenario: Scenario) -> list[MetricResult]:
...
class JvmScenarioMetricsMixin(ScenarioMetrics):
    ...
    def jvm_threads(self, scenario: Scenario) -> list[MetricResult]:
    ...
    def jvm_memory(self, scenario: Scenario) -> list[MetricResult]:
    ...
    kubernetes_pod_name=~".*{scenario.pod_part}.*"
        ...
        jvm_mem_bytes_committed_df = self._query_range(
                jvm_mem_bytes_committed_q, scenario)
    ...
    def jvm_gc(self, scenario: Scenario) -> list[MetricResult]:
    ...
    kubernetes_pod_name=~".*{scenario.pod_part}.*"
        ...
        jvm_gc_sum_df = self._query_range(jvm_gc_collection_seconds_sum_q,
                                          scenario,
                                          {
                                              'metric_name': 'jvm_gc_collection_seconds_sum_1m'})
        jvm_gc_count_df = self._query_range(jvm_gc_collection_seconds_count_q,
                                            scenario,
                                            {
...
class ContainerScenarioMetricsMixin(ScenarioMetrics):
    ...
    def cluster_size(self, scenario: Scenario) -> list[MetricResult]:
    ...
    def container_cpu_utilization(self, scenario: Scenario) -> list[
        MetricResult]:
...
sum(container_spec_cpu_quota{{
    ...
    def disk_bytes(self, scenario) -> list[MetricResult]:
    ...
    pod=~".*{scenario.pod_part}.*"
        ...
        disk_bytes_reads_df = self._query_range(container_fs_read_bytes_total_q,
                                                scenario, {
    ...
    def disk_io(self, scenario: Scenario) -> list[MetricResult]:
    ...
    pod=~".*{scenario.pod_part}.*",
        ...
        disk_io_reads = self._query_range(container_fs_reads_total_q, scenario,
                                          {
                                              'metric_name': 'container_fs_reads_total_1m'})
        disk_io_writes = self._query_range(container_fs_writes_total_q,
                                           scenario,
                                           {
    ...
    def network_bytes(self, scenario: Scenario) -> list[MetricResult]:
